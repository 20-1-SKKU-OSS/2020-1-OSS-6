...
| Detecting and removing a cycle in linkedlist.|
| Determine if a linked list is a pallindrome. |
| Insert data in a sorted linked list.|
| Determine the intersection(merging) point of two given linked list.|
...
| Given a singly linked list L<sub>0</sub> -> L<sub>1</sub> -> … -> L<sub>n-1</sub> -> L<sub>n</sub>. Rearrange the nodes in the list (in place) so that the new formed list is : L<sub>0</sub> -> L<sub>n</sub> -> L<sub>1</sub> -> L<sub>n-1</sub> -> L<sub>2</sub> -> L<sub>n-2<sub> .... |
...
| :------------: |
| Generic Macros and Algorithms like swap, random number generation |
...
| Linux Kernel Double LinkedList Implementation |
| Generic Graph Implementation (Adjacency List) |
